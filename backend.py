...
def store_vector(item: VectorItem):
    database.append(item)
    return {"message": "stored", "count": len(database)}
...
def search_vectors(req: SearchRequest):
    ...
    q = np.array(req.vector)

    results = []

    for item in database:
        s = np.array(item.vector)

        max_len = max(len(q), len(s))

        q_p = np.pad(q, (0, max_len - len(q)))
        s_p = np.pad(s, (0, max_len - len(s)))

        score = float(cosine_similarity([q_p], [s_p])[0][0])
